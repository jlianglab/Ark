...
# python main_ark.py  --data_set ProstateMRI --opt adam --warmup-epochs 20  --lr 0.2 --batch_size 128 --model unet --init random --pretrain_epochs 1000  --momentum_teacher 0.9  --exp_name avg4ep_lr02_500ep
def get_args_parser():
    parser = OptionParser()
    ...
    parser.add_option("--num_class", dest="num_class", help="number of the classes in the downstream task",
                      default=1, type="int")
    parser.add_option("--data_set", dest="dataset", default="ProstateMRI", type="string")
...
def main(args):
    ...
    for source in source_list:
        ...
        dataset_val_list.append(
            dict_dataloarder[dataset](data_path=datasets_config[dataset]['data_dir'], split = 'val', source = source, crop_size=args.crop_size, resize=args.resize, augment=build_transform_classification(normalize=args.normalization, crop_size=args.crop_size, resize=args.resize, mode="valid"))
        )
