from utils import MetricLogger, ProgressLogger, save_image, DiceLoss
import time
import torch
...
def locked_train_one_epoch(model, use_head_n, dataset, data_loader_train, device, criterion, optimizer, epoch, ema_mode, teacher, momentum_schedule, it):
    
    # freeze all layers except the first layer of the model
...
def train_one_epoch(model, use_head_n, dataset, data_loader_train, device, criterion, optimizer, epoch, ema_mode, teacher, momentum_schedule, it):
    ...
    for i, (samples1, samples2, targets) in enumerate(data_loader_train):
        ...
        if i % 50 == 0:
            progress.display(i)
            # save_image(samples[0].cpu().numpy().transpose(1, 2, 0), "Models/student"+str(i))
            # save_image(samples[0].cpu().numpy().transpose(1, 2, 0),"Models/teacher"+str(i))
...
def test(model, use_head_n, data_loader_test, device): 
    ...
    with torch.no_grad():
        for i, (samples, _, targets) in enumerate(tqdm(data_loader_test)):
            ...
            _, output = model(samples,use_head_n)
            test_dice += DiceLoss().dice_coef(output, targets).item()
            
    dice = test_dice/ len(data_loader_test)
    return dice


def test_classification(model, use_head_n, data_loader_test, device, multiclass = True): 
