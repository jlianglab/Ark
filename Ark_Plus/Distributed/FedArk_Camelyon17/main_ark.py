...
#python main_ark.py  --data_set Camelyon17 --opt sgd --warmup-epochs 20  --lr 0.3 --batch_size 200 --model densenet --init random --pretrain_epochs 200  --momentum_teacher 0.9  --projector_features 1376  --exp_name 5clients_lr0.3 --resume True
def get_args_parser():
    parser = OptionParser()
    ...
    parser.add_option("--num_class", dest="num_class", help="number of the classes in the downstream task",
                      default=1, type="int")
    parser.add_option("--data_set", dest="dataset", default="Camelyon17", type="string")
    parser.add_option("--normalization", dest="normalization", help="how to normalize data (imagenet|chestx-ray)", default="imagenet",
                      type="string")
    parser.add_option("--input_size", dest="crop_size", help="input resolution", default=96, type="int")
    parser.add_option("--img_resize", dest="resize", help="resize image resolution", default=112, type="int")
...
def main(args):
    ...
    source_list = datasets_config[dataset]['sources']
    client_list = datasets_config[dataset]['clients']
    
    for source in source_list:
        dataset_train_list.append(
            dict_dataloarder[dataset](data_path=datasets_config[dataset]['data_dir'], split = 'train', source = source, crop_size=args.crop_size, resize=args.resize, augment=None)
        )
        dataset_val_list.append(
            dict_dataloarder[dataset](data_path=datasets_config[dataset]['data_dir'], split = 'test', source = source, crop_size=args.crop_size, resize=args.resize, augment=build_transform_classification(normalize=args.normalization, crop_size=args.crop_size, resize=args.resize, mode="valid"))
        )
        dataset_test_list.append(
            dict_dataloarder[dataset](data_path=datasets_config[dataset]['data_dir'], split = 'test', source = source, crop_size=args.crop_size, resize=args.resize, augment=build_transform_classification(normalize=args.normalization, crop_size=args.crop_size, resize=args.resize, mode="test"))
        )

    omni_engine(args, model_path, output_path, source_list, client_list, datasets_config, dataset_train_list, dataset_val_list, dataset_test_list)
