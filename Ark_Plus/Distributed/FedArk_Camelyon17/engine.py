...
def omni_engine(args, model_path, output_path, source_list, client_list, datasets_config, dataset_train_list, dataset_val_list, dataset_test_list):
    ...
    if args.mode == "train":
        ...
        for epoch in range(start_epoch, args.pretrain_epochs):
            ...
            client_count = 0
            start_epochs = datasets_config[args.dataset]['start_epoch']
            for start_ep in start_epochs:
                if epoch >= start_ep:
                    client_count +=1
            print("Number of active clients at epoch {}: {}".format(epoch, client_count))
            
            for c, client_ds in enumerate(client_list[:client_count]):
                ...
                for i in client_ds:
                    ...
                    train_one_epoch(student, 0, source_list[i], data_loader_list_train[i], device, criterion, optimizer, epoch, args.ema_mode, teachers[c], momentum_schedule, it-start_epochs[c])

                for i in client_ds:
                    val_loss = evaluate(teachers[c], 0, data_loader_list_val[i], device, criterion, source_list[i])
                ...
                with torch.no_grad():
                    for (name, param_q), param_k in zip(student.named_parameters(), master.parameters()):
                        param_k.data.add_((1.0/client_count) * param_q.detach().data)
            ...
            if epoch % args.test_epoch == 0 or epoch+1 == args.pretrain_epochs:
                ...
                with open(output_file, 'a') as writer:
                    ...
                    for c, client_ds in enumerate(client_list):
                        for i in client_ds:
                            ...
                            y_test, p_test = test_classification(master, 0, data_loader_list_test[i], device)
                            y_test_teacher, p_test_teacher = test_classification(teachers[c], 0, data_loader_list_test[i], device)
                        # if multiclass:
                            ...
                            print(">>{}:Central Master ACCURACY = {} \nLocal Teacher ACCURACY = {}\n".format(source_list[c], acc, acc_teacher))
                            writer.write(
                                "\n{}:Central Master ACCURACY = {} \nLocal Teacher ACCURACY = {}\n".format(source_list[c], np.array2string(np.array(acc), precision=4, separator='\t'), np.array2string(np.array(acc_teacher), precision=4, separator='\t')))   
                            t_res.append(acc)
                ...
                print("\nCentral Master meanAUC = \n{} \n Local Teacher meanAUC = \n{}\n".format(test_results, test_results_teacher))
        with open(output_file, 'a') as writer:
