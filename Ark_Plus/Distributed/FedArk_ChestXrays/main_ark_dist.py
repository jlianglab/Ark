...
def get_args_parser():
    ...
    parser.add_option("--client", dest="client_list", help="CheXpert|ChestXray14, Shenzhen|RSNAPneumonia,VinDrCXR",  action="append")
...
def main(args):
    ...
    dataset_list, client_list = [],[]
    idx = 0
    for client in args.client_list:
        client_ds_idx = []
        for dataset in client.split(','):
            assert dataset in list(datasets_config.keys())
            dataset_list.append(dataset)
            client_ds_idx.append(idx)
            idx += 1
        client_list.append(client_ds_idx)
    print(dataset_list)
    print(client_list)
    ...
    omni_engine(args, model_path, output_path, dataset_list, client_list, datasets_config, dataset_train_list, dataset_val_list, dataset_test_list)
