...
def omni_engine(args, model_path, output_path, dataset_list, datasets_config, dataset_train_all, dataset_val_list, dataset_test_list):
    device = torch.device(args.device)
    cudnn.benchmark = True

    # logs
    exp = 'Ablation_Concurrent'
    ...
    data_loader_train = DataLoader(dataset=dataset_train_all, batch_size=args.batch_size, shuffle=True,
    ...
    if args.ema_mode == "epoch":
        ...
        iters_per_epoch = int(len(dataset_train_all) / args.batch_size) + 1
        momentum_schedule = cosine_scheduler(args.momentum_teacher, 1,
                                               args.pretrain_epochs, iters_per_epoch)
        print(iters_per_epoch, len(momentum_schedule))
    ...
    if args.mode == "train":
        ...
        it = start_epoch * len(dataset_list) if args.ema_mode == "epoch" else start_epoch * iters_per_epoch
        
        for epoch in range(start_epoch, args.pretrain_epochs):
            train_one_epoch(model, data_loader_train, num_classes_list, device, criterion, optimizer, epoch, args.ema_mode, teacher, momentum_schedule, it)
