...
def get_args_parser():
    ...
    parser.add_option("--ema_mode", dest="ema_mode", default="iteration", help="update teacher model at which time (epoch | iteration)", type="string")
    ...
    # parser.add_option("--reinit_heads", dest="reinit_heads", help="whether re-initialize the heads from pre-trained model", default=False, action="callback",
    #                   callback=vararg_callback_bool)
...
def main(args):
    ...
    omni_datasets = OmniPretrainingDatasets(datasets_config = datasets_config, dataset_list = args.dataset_list)
    ...
    omni_engine(args, model_path, output_path, args.dataset_list, datasets_config, omni_datasets, dataset_val_list, dataset_test_list)
