...
def omni_engine(args, model_path, output_path, dataset_list, datasets_config, dataset_train_list, dataset_val_list, dataset_test_list):
    ...
    # Consolidate labels from 5 datasets: CheXpert, ChestX-ray14, RSNAPneumonia, VinDrCXR, Shenzhen
    targets = ['No Finding', 'Enlarged Cardiomediastinum', 'Cardiomegaly', 'Lung Opacity', 'Lung Lesion', 'Edema', 'Consolidation', 'Pneumonia', 'Atelectasis', 'Pneumothorax', 'Pleural Effusion', 'Pleural Other', 'Fracture', 'Support Devices',
               'Infiltration', 'Mass', 'Nodule', 'Emphysema', 'Fibrosis', 'Pleural_Thickening', 'Hernia', 
               'No Lung Opacity/Not Normal', 'Lung tumor','Tuberculosis', 'Other diseases']
    
    # Consolidate labels from 2 datasets: ChestX-ray14, VinDrCXR
    # targets = ['No Finding','Cardiomegaly', 'Edema', 'Consolidation', 'Pneumonia', 'Atelectasis', 'Pneumothorax', 'Pleural Effusion', 
    #            'Infiltration', 'Mass', 'Nodule', 'Emphysema', 'Fibrosis', 'Pleural_Thickening', 'Hernia', 
    #             'Lung tumor','Tuberculosis', 'Other diseases']

    # logs
    exp = 'Ablation_SingleHead'
    ...
    # training setups
    model = build_omni_model(args, len(targets))
    teacher = build_omni_model(args, len(targets))     
    print(model)
    ...
    if args.mode == "train":
        ...
        for epoch in range(start_epoch, args.pretrain_epochs):
            for i, data_loader in enumerate(data_loader_list_train): 
                ...
                targets_idx = [targets.index(c) for c in datasets_config[dataset_list[i]]['diseases']]
                print(len(targets_idx), targets_idx)
                train_one_epoch(model, targets_idx, dataset_list[i], data_loader, device, criterion, optimizer, epoch, args.ema_mode, teacher, momentum_schedule, it)
                it += 1
            val_loss_list = []
            for i, dv in enumerate(data_loader_list_val):
                criterion = torch.nn.CrossEntropyLoss() if datasets_config[dataset_list[i]]['task_type'] == "multi-class classification" else torch.nn.BCEWithLogitsLoss()
                targets_idx = [targets.index(c) for c in datasets_config[dataset_list[i]]['diseases']]
                val_loss = evaluate(model, targets_idx, dv, device, criterion, dataset_list[i])
            ...
            if epoch % args.test_epoch == 0 or epoch+1 == args.pretrain_epochs:
                ...
                with open(output_file, 'a') as writer:
                    ...
                    for i, dataset in enumerate(dataset_list):
                        ...
                        targets_idx = [targets.index(c) for c in diseases]
                        y_test, p_test = test_classification(model, targets_idx, data_loader_list_test[i], device, multiclass)
                        y_test_teacher, p_test_teacher = test_classification(teacher, targets_idx, data_loader_list_test[i], device, multiclass)
                        if multiclass:
